##this is for the task s-227-100
import time 
...
class Standardised_reports_models:
    def standardised_reports_models():
...
#                     print(html,"this is html")
                ...
                for row in rows:
                        ...
                        if quest in guidance_text :
                            ...
                            guidance_button = row.find_element(By.XPATH, ".//a[@name='view_model']")
                ...
                driver.find_element(By.NAME ,'rating').send_keys(short)
                driver.find_element(By.XPATH ,'/html/body/section/div/div[2]/div/div/div/form/div[2]/div[2]/div/div/div/div[3]/div[2]').send_keys(short)
                time.sleep(2)
                ...
                # Wait for the table to be visible
                table = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.ID, "shortanswer_table")))
                ...
                upd = str(unique_id)
                print(upd)
                short = driver.find_element(By.NAME ,'rating')
                short.clear()
                # time.sleep(1)
                short.send_keys(upd)
                time.sleep(1)
                ...
                # except:
                #      pass
                # desc = driver.find_element(By.XPATH ,'/html/body/section/div/div[2]/div/div/div/form/div[2]/div[2]/div/div/div/div[3]/div[2]')
                
                # desc.clear()
                # desc.send_keys(upd)
                
                button = driver.find_element(By.NAME ,'btn_update')
                driver.execute_script("arguments[0].click();", button)
...
#                 print("button not  exists and no error")
    ...
    # Save the DataFrame to an Excel file
        unique_id = random.randint(1000, 9999)
        excelsave = "SR_Models"+str(unique_id)+".xlsx"

...
#                     print(html,"this is html")
                ...
                # Find the row containing the "new_decision"
                row = driver.find_element(By.XPATH, row_xpath)

                # Perform the desired actions on the row, e.g., click on the "Edit" button
                edit_button = row.find_element(By.ID, "edit")
                edit_button.click()
                time.sleep(2)
                new_decision = driver.find_element(By.NAME,'subdec_name')
                new_decision.clear()
                new_decision.send_keys('new_decision_update')
                new_decision_description = driver.find_element(By.NAME,'subdec_desc')
                new_decision_description.clear()
                new_decision_description.send_keys('new_decision_description_update')
                time.sleep(0.2)
                driver.find_element(By.ID,'btnsb').click()

                time.sleep(3)
                name = "new_decision_update"
                row_xpath = f"//tr[td[b[contains(text(), '{name}')]]]"

                # Find the row containing the "new_decision"
                row = driver.find_element(By.XPATH, row_xpath)

                # Perform the desired actions on the row, e.g., click on the "delete" button
                delete_button = row.find_element(By.ID, "delete")
                delete_button.click()
                time.sleep(2)
...
#                 print("button not  exists and no error")
    ...
    # Save the DataFrame to an Excel file
        ...
        excelsave = "sub_decisions"+str(unique_id)+".xlsx"
